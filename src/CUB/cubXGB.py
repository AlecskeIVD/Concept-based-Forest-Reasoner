...
from src.CUB.cubDataset import createPrecomputedDataloaders
...
class cubConceptPredictor(pl.LightningModule):
    def __init__(self, encoder, embedding_size=2048, num_classes=50):
        super(cubConceptPredictor, self).__init__()
...
def runXGBExperiment():
    ...
    batchSize = 32
    depths = [7, 5, 3]
    memorySizes = [200, 100, 50, 20]
    maxEpochs = 50
    torch.manual_seed(42) 
    train_loader, val_loader, test_loader = createPrecomputedDataloaders(
        batchSize=32, # You can use a large batch size here since no GPU/Images are needed
        embeddings_file="/Users/alecvandeuren/Thesis/src/data/raw/CUB/CUB_BASE/CUB_200_2011/cub_embeddings.pt",
    )
    ...
    for depth, mem in product(depths, memorySizes):
        ...
        for trial in range(3):
            print(f"\nRunning XGBoost with max_depth={depth}, maxTrees={mem}, trial {trial+1}/3")
            ...
            conceptPredictor = cubConceptPredictor(encoder=encoder, embedding_size=EMB_SIZE, num_classes=nConcepts)
            ...
            conceptPredictor = cubConceptPredictor.load_from_checkpoint(checkpoint_cb.best_model_path, encoder=encoder, embedding_size=EMB_SIZE, num_classes=nConcepts)
            ...
            print(f"Validation Accuracy for XGBoost with max_depth={depth}, maxTrees={mem}, trial {trial+1}/3: {acc * 100:.2f}%")
        ...
        print(f"\nAverage Validation Accuracy for max_depth={depth}, maxTrees={mem}: {avg_acc * 100:.2f}% ± {stdAccuracy * 100:.2f}%")
...
"""
max_depth=5, max_trees=20: 9.37% ± 0.80%
max_depth=3, max_trees=100: 9.37% ± 0.96%
max_depth=3, max_trees=20: 9.29% ± 1.09%
max_depth=3, max_trees=50: 9.65% ± 1.47%
max_depth=5, max_trees=100: 9.17% ± 1.00%
max_depth=5, max_trees=200: 9.17% ± 1.00%
max_depth=7, max_trees=20: 9.29% ± 1.13%
max_depth=5, max_trees=50: 9.17% ± 1.15%
max_depth=3, max_trees=200: 9.01% ± 1.00%
max_depth=7, max_trees=50: 8.87% ± 0.93%
max_depth=7, max_trees=100: 7.81% ± 0.35%
max_depth=7, max_trees=200: 7.81% ± 0.35%"""
...
def ComputeAccuracy():
    ...
    for depth, mem in product(depths, memorySizes):
        ...
        for trial in range(3):
            ...
            print(f"Test Accuracy for XGBoost with max_depth={depth}, maxTrees={mem}, trial {trial+1}/3: {acc * 100:.2f}%")
            accuracies.append(acc)
        avg_acc = np.mean(accuracies)
        stdAccuracy = (sum([(x-avg_acc)**2 for x in accuracies])/len(accuracies))**0.5
        allAccuracies.append((avg_acc, stdAccuracy, (depth, mem)))
        print(f"\nAverage Test Accuracy for max_depth={depth}, maxTrees={mem}: {avg_acc * 100:.2f}% ± {stdAccuracy * 100:.2f}%")
    print("\nSummary of all configurations:")
    allAccuracies.sort(key=lambda x: x[0]-x[1], reverse=True)
    for avg_acc, std_acc, (depth, mem) in allAccuracies:
        print(f"max_depth={depth}, max_trees={mem}: {avg_acc * 100:.2f}% ± {std_acc * 100:.2f}%")
    # Average Test Accuracy for max_depth=5, maxTrees=20: 9.45% ± 0.33%
